 ...
 *  @brief 
 */
typedef void (*menu_item_callback_t) (int);
...
void Menu_Init(void)
{
    s_menu_add_item ("Start",   NULL, NULL, 0);
    s_menu_add_item ("Test",    NULL, NULL, 0);
    ...
    s_menu_add_item ("Enable",     menu_lo_arm, NULL, 0);
    s_menu_add_item ("Delay",      menu_lo_arm, NULL, 0);
    s_menu_add_item ("Duration",   menu_lo_arm, NULL, 0);
    s_menu_add_item ("PWM Enable", menu_lo_arm, NULL, 0);
    s_menu_add_item ("Period 1",   menu_lo_arm, items_set_lo_pwm_period01, 0);
    s_menu_add_item ("Period 10",  menu_lo_arm, items_set_lo_pwm_period10, 0);
    s_menu_add_item ("Period 25",  menu_lo_arm, items_set_lo_pwm_period25, 0);
    s_menu_add_item ("Period 50",  menu_lo_arm, items_set_lo_pwm_period50, 0);
    ...
    s_menu_add_item ("Enable",     menu_hi_arm, NULL, 0);
    s_menu_add_item ("Delay",      menu_hi_arm, NULL, 0);
    s_menu_add_item ("Duration",   menu_hi_arm, NULL, 0);
    s_menu_add_item ("PWM Enable", menu_hi_arm, NULL, 0);
    s_menu_add_item ("Period 1",   menu_hi_arm, items_set_hi_pwm_period01, 0);
    s_menu_add_item ("Period 10",  menu_hi_arm, items_set_hi_pwm_period10, 0);
    s_menu_add_item ("Period 25",  menu_hi_arm, items_set_hi_pwm_period25, 0);
    s_menu_add_item ("Period 50",  menu_hi_arm, items_set_hi_pwm_period50, 0);
...
static void s_rotary_encoder_callback (uint32_t current)
{
    ...
    if (s_menu_handle.current->callback != NULL && (s_menu_handle.current->flags & MENU_FLAG_EDIT_DATA) == MENU_FLAG_EDIT_DATA)
    {
        s_menu_handle.current->callback(s_menu_handle.rotenc.delta);
    } else {
...
static void s_push_button_callback (void)
{
    ...
    else if (s_menu_handle.current->parent && (s_menu_handle.current->flags & MENU_FLAG_GOTO_PARENT) == MENU_FLAG_GOTO_PARENT)
    {
        ...
        // Обновление отображения меню
        s_display_menu();
    } else if (s_menu_handle.current->callback != NULL)
    {
        if ((s_menu_handle.current->flags & MENU_FLAG_EDIT_DATA) == MENU_FLAG_EDIT_DATA)
        {
            s_menu_handle.current->flags &= ~MENU_FLAG_EDIT_DATA;
            s_display_menu();
        } else
        {
            s_menu_handle.current->flags |= MENU_FLAG_EDIT_DATA;
            s_menu_handle.current->callback(0);
        }
    }
}
