...
menu_node_t * menu_navigate_delta(int16_t delta)
{
    ...
    if (delta > 0)
    {
        ...
        {
            s_context.current = menu_next_cycle(s_context.current);
        }
    } else {
        for (int i = 0; i < -delta; i++)
        {
            s_context.current = menu_prev_cycle(s_context.current);
        }
    }
...
menu_node_t *menu_next_cycle(menu_node_t *node) {
    return node->next ? node->next : node->parent->first_child;
}

menu_node_t *menu_prev_cycle(menu_node_t *node) {
...
const char * menu_get_next_title(void)
{
    ...
    if (s_context.current == 0)
        return NULL;
    menu_node_t *next = menu_next_cycle(s_context.current);
    if (next == NULL)
        return NULL;
    return next->title;
}
